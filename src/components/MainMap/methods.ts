...
export const _limitedNewScale = ({
  ...
  if (
    ...
    newScale >=
      initScale *
        maxDynamicFinal *
        (role === "mobile" ? 2 : initScale >= 1 ? 1.6 : 1)
  ) {
    return {
      value:
        initScale *
  ...
  return section?.rows.forEach((row: Row) => {
    row?.seats.forEach((seat: Seat) => {
      ...
      if (seat.x >= x1 && seat.x <= x2 && seat.y >= y1 && seat.y <= y2) {
        ...
        // final paint
        newSeatCircle.radius(
          role !== "mobile" ? (Number(seat.name) < 100 ? 4 : 3.7) : 3.9
        );
